...
from core.recovery.regions import get_region_box, list_region_names
...
def parse_region(args: argparse.Namespace) -> Tuple[float, float, float, float]:
    if args.region:
        return get_region_box(args.region)
    ...
    if None not in (args.x1, args.y1, args.x2, args.y2):
        ...
        y2 = clamp01(args.y2)
        if x2 <= x1 or y2 <= y1:
            raise ValueError("invalid custom region, require x2>x1 and y2>y1")
        return x1, y1, x2, y2

    raise ValueError("must provide --region or all of --x1 --y1 --x2 --y2")


def crop_region(image_path: str, region_pct: Tuple[float, float, float, float], out_crop_path: str) -> Dict[str, Any]:
...
def map_crop_boxes_to_full_image(crop_boxes: List[Dict[str, Any]], crop_meta: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for b in crop_boxes:
        ...
        row = {
            "text": b.get("text", ""),
            "x1": clamp01(fx1 / full_w),
            "y1": clamp01(fy1 / full_h),
            "x2": clamp01(fx2 / full_w),
            "y2": clamp01(fy2 / full_h),
        }
        if "score" in b:
            row["score"] = b["score"]
...
def draw_debug(image_path: str, region_pct: Tuple[float, float, float, float], full_boxes: List[Dict[str, Any]], out_debug_path: str) -> None:
    with Image.open(image_path) as im:
        w, h = im.size
...
def main() -> None:
    ...
    parser.add_argument("--region", choices=list_region_names())
    ...
    if provider is None or not hasattr(provider, "recognize_with_boxes"):
        raise RuntimeError("No OCR provider with boxes mode available")

    result = provider.recognize_with_boxes(crop_path)  # type: ignore[attr-defined]
    crop_boxes = result.boxes or []
    full_boxes = map_crop_boxes_to_full_image(crop_boxes, crop_meta)
