...
from core.recovery.action_semantics import default_regions_for_action
from core.recovery.builtin_aliases import BUILTIN_ALIASES, get_builtin_aliases
...
class ClickResolver:
    ...
    def resolve_semantic(
        ...
        *,
        ...
        legacy_disable_ocr = str(os.getenv("CLICK_DISABLE_OCR", "")).strip().lower() in {"1", "true", "yes"}
        disable_pre_ai_ocr = legacy_disable_ocr or str(os.getenv("CLICK_DISABLE_PRE_AI_OCR", "")).strip().lower() in {"1", "true", "yes"}
        disable_post_ai_region_ocr = legacy_disable_ocr or str(os.getenv("CLICK_DISABLE_POST_AI_REGION_OCR", "")).strip().lower() in {"1", "true", "yes"}
        ...
        if (not disable_pre_ai_ocr) and pack.image_path and self._should_retry_region_ocr(
            primary_target, alias_pool, target_role, full_ocr_candidates
        ):
        ...
        ai = self._resolve_by_ai_semantic(
            pack, primary_target, logical_name, alias_pool, preferred_regions, target_role=target_role
        )
        ...
        if ai and (not disable_post_ai_region_ocr):
            ...
            rerun = self._resolve_by_region_ocr(
                pack,
                rerun_targets,
                rerun_regions,
                target_role=target_role,
    ...
    def _default_regions_for_role(
        self,
        alias_pool: List[str],
        target_role: str,
        preferred_regions: Optional[List[str]] = None,
    ) -> List[str]:
        page_mode = "dialog" if any(
            str(x or "").startswith("dialog_") or str(x or "") in {"center_dialog", "bottom_sheet_area"}
            for x in (preferred_regions or [])
        ) else "default"
        return default_regions_for_action(
    ...
    def _build_alias_pool(
        ...
        *,
        ...
        builtin_aliases = get_builtin_aliases(logical_name)
        for a in builtin_aliases:
            add_text(a)

        for name, aliases in BUILTIN_ALIASES.items():
        ...
        if not preferred_regions:
            preferred_regions = self._default_regions_for_role(alias_pool, target_role, explicit_regions)
        if not preferred_regions:
    ...
    def _resolve_by_page_source_candidates(
        ...
        *,
        ...
        for node in nodes:
            ...
            if x2 <= x1 or y2 <= y1:
                continue
            region_penalty = 0.0 if self._within_preferred_regions((x1 + x2) / 2.0, (y1 + y2) / 2.0, preferred_regions) else 0.12
            ...
            if target_role != "input" and not is_clickable:
                continue
            cx, cy = self._pick_click_point_from_box(
                text_value or desc_value or rid_value, x1, y1, x2, y2, preferred_regions, target_role=target_role
            )
    ...
    def _resolve_by_region_ocr(
        ...
        *,
        ...
        for region_name in regions[:3]:
            ...
            temp_pack = PerceptionPack(
                image_path=image_path,
                ocr_text=region_result.get("text", ""),
                meta={**meta, "ocr_boxes": full_boxes},
            )
        ...
        *,
        ...
        try:
            parsed = self.policy_runner.decide_next(
                ...
                recent={},
                hints={
                    "prefer_semantic_click": True,
                    "overlay_hints": region_hints,
                    "target_role": target_role,
                    "prefer_region": region_hints[0] if region_hints else None,
                },
            )
        except Exception:
            return None
    ...
    def _run_region_ocr(self, provider: Any, image_path: str, bounds: Tuple[float, float, float, float]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for b in crop_boxes:
                ...
                row = {
                    "text": b.get("text", ""),
                    "x1": self._clamp01(fx1),
                    "y1": self._clamp01(fy1),
                    "x2": self._clamp01(fx2),
                    "y2": self._clamp01(fy2),
                }
                if "score" in b:
                    row["score"] = b["score"]
    ...
    def _parse_bounds(self, raw: str, meta: Dict[str, Any]) -> Dict[str, float]:
        ...
        return {
            "x1": self._clamp01(x1 / sw),
            "y1": self._clamp01(y1 / sh),
            "x2": self._clamp01(x2 / sw),
            "y2": self._clamp01(y2 / sh),
        }
